...
static const double w_slacks = 2000.00;
...
static void lsqfcn_min_tau_and_max_time (
	...
	if (info->cimos > 2) {  // for limit cycle stage
		res[0] = sK * sw_tau * tau * discount;
		res[1] = 1.0 * sK * sw_time * discount * cos(sd_time) ;
	} else {  // for transition stage
		res[0] = sw_tau * tau * discount;
		res[1] = 1.0 * sw_time * discount * cos(sd_time) ;
	}
...
static const unsigned int LSQFCN_SLACKS = 4 + 1;
...
void def_model(void) {
	...
	// define LSQ objective
	def_lsq(imos, "s", NPR,
		LSQFCN_TRACK_VREF_WREG_AND_MAX_TIME_NE,
		lsqfcn_track_vref_wreg_and_max_time
	);
	def_lsq(imos, "i", NPR,
