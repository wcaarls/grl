...
*
* void RandomInit(rg_uint32 seed);
* Re-initializes the random number generator with a new seed.
*
* void RandomInitByArray(rg_uint32 seeds[], int length);
...
#include <stdint.h>
typedef	int32_t		rg_int32;
typedef	uint32_t	rg_uint32;
...
class TRandomMersenne {                // encapsulate random number generator
  ...
  #endif
  public:
  TRandomMersenne(rg_uint32 seed) {       // constructor
    RandomInit(seed);}
  void RandomInit(rg_uint32 seed);        // re-seed
  void RandomInitByArray(rg_uint32 seeds[], int length); // seed by more than 32 bits
  int IRandom(int min, int max);       // output random integer
  double Random();                     // output random float
  rg_uint32 BRandom();                    // output random bits
  private:
  rg_uint32 mt[MERS_N];                   // state vector
...
class TRanrotBGenerator {              // encapsulate random number generator
  ...
  public:
  void RandomInit(rg_uint32 seed);        // initialization
  int IRandom(int min, int max);       // get integer random number in desired interval
  double Random();                     // get floating point random number
  rg_uint32 BRandom();                    // output random bits
  TRanrotBGenerator(rg_uint32 seed);      // constructor
  protected:
  int p1, p2;                          // indexes into buffer
  rg_uint32 randbuffer[KK];               // history buffer
  rg_uint32 randbufcopy[KK*2];            // used for self-test
  enum TArch {LITTLEENDIAN, BIGENDIAN, NONIEEE};
  TArch Architecture;                  // conversion to float depends on computer architecture
};

// Declaration of global ranrotB generator
extern TRanrotBGenerator	gRanrotB;
...
class TRanrotWGenerator {              // encapsulate random number generator
  ...
  rg_uint32 BRandom();                    // output random bits
  TRanrotWGenerator(rg_uint32 seed);      // constructor
  protected:
  int p1, p2;                          // indexes into buffer
  union {                              // used for conversion to float
    long double randp1;
    rg_uint32 randbits[3];};
  rg_uint32 randbuffer[KK][2];            // history buffer
  rg_uint32 randbufcopy[KK*2][2];         // used for self-test
...
class TRandomMotherOfAll {             // encapsulate random number generator
  ...
  int IRandom(int min, int max);       // get integer random number in desired interval
  double Random();                     // get floating point random number
  TRandomMotherOfAll(rg_uint32 seed);     // constructor
  protected:
