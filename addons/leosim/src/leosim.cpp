...
void LeoSimEnvironment::configure(Configuration &config)
{
  ...
  // reserve memory
  ode_obs_.v.resize(ode_observation_dims_);
  ode_action_.v.resize(ode_action_dims_);
}
...
double LeoSimEnvironment::step(const Action &action, Observation *obs, double *reward, int *terminal)
{
  ...
  if (bhWalk_.stanceLegLeft())
    ode_action_.v << actionArm, actionSwingHip, actionStanceHip, actionSwingKnee, actionStanceKnee, actionAnkles;
  else
    ode_action_.v << actionArm, actionStanceHip, actionSwingHip, actionStanceKnee, actionSwingKnee, actionAnkles;
