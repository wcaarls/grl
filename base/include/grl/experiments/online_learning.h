...
class OnlineLearningExperiment : public Experiment
{
  ...
  protected:
    ...
    int test_interval_, test_trials_;
    double rate_;
    std::string identity_, output_, load_file_;
    std::string save_every_;

  public:
    OnlineLearningExperiment() : agent_(NULL), test_agent_(NULL), environment_(NULL), exporter_(NULL), state_(NULL), action_(NULL), reward_(NULL), curve_(NULL), runs_(1), run_offset_(0), trials_(0), steps_(0), test_interval_(-1), test_trials_(1), rate_(0), save_every_("never")  { }
