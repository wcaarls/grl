...
bool CLeoBhWalkSym::readConfig(const CConfigSection &configNode)
{
  bool configresult = CSTGAgentQLeo::readConfig(configNode);
  ...
  configresult &= mLogAssert(configNode.get("preprogrammedStanceKneeAngle", &mPreProgStanceKneeAngle));
  configresult &= mLogAssert(configNode.get("preprogrammedEarlySwingTime", &mPreProgEarlySwingTime));
  configresult &= mLogAssert(configNode.get("preprogrammedExploreRate", &mPreProgExploreRate));
