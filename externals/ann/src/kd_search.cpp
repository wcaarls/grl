	...
	double				eps)			// the error bound
{
	...
	m_params.kdDim = dim;						// copy arguments to static equivs
	m_params.kdQ = q;
	m_params.kdPts = pts;
	...
	m_params.kdMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating op count

#ifndef ANN_REALTIME
	m_params.kdPointMK = new ANNmin_k(k);		// create set for closest k points
#else
	m_params.kdPointMK->reset(k);		// create set for closest k points
#endif
										// search starting at the root
	root->ann_search(&m_params, annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim));

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = m_params.kdPointMK->ith_smallest_key(i);
		nn_idx[i] = m_params.kdPointMK->ith_smallest_info(i);
	}
#ifndef ANN_REALTIME
	delete m_params.kdPointMK;				// deallocate closest point set
#endif
}
...
void ANNkd_split::ann_search(ANNparams *params, ANNdist box_dist)
{
										// check dist calc term condition
	if (g_maxPtsVisited != 0 && params->ptsVisited > g_maxPtsVisited) return;

										// distance to cutting plane
	ANNcoord cut_diff = params->kdQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_search(params, box_dist);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - params->kdQ[cut_dim];
		...
		if (box_dist * params->kdMaxErr < params->kdPointMK->max_key())
			child[ANN_HI]->ann_search(params, box_dist);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_search(params, box_dist);// visit closer child first

		ANNcoord box_diff = params->kdQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * params->kdMaxErr < params->kdPointMK->max_key())
			child[ANN_LO]->ann_search(params, box_dist);
...
void ANNkd_leaf::ann_search(ANNparams *params, ANNdist box_dist)
{
	...
	min_dist = params->kdPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = params->kdPts[bkt[i]];			// first coord of next data point
		qq = params->kdQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < params->kdDim; d++) {
		...
		if (d >= params->kdDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			params->kdPointMK->insert(dist, bkt[i]);
			min_dist = params->kdPointMK->max_key();
		}
	}
