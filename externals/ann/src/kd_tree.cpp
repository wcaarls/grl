...
//	one tree).
//----------------------------------------------------------------------
static int				g_IDX_TRIVIAL[] = {0};	// trivial point index
ANNkd_leaf				*g_KD_TRIVIAL = NULL;		// trivial leaf node
...
int ANNkd_split::getByteSize()
{
	int result = sizeof(*this);
	if (child[0] != NULL)
		result += child[0]->getByteSize();
	if (child[1] != NULL)
		result += child[1]->getByteSize();
	return result;
}
		...
		ostream &out)					// output stream
{
	...
	if (this == g_KD_TRIVIAL) {			// canonical trivial leaf node
		out << "Leaf (trivial)\n";
	}
	else{
		out << "Leaf n=" << n_pts << " <";
	...
	ANNorthRect			&bnd_box)				// bounding box
{
	...
	if (this == g_KD_TRIVIAL) st.n_tl = 1;		// count trivial leaf
...
//----------------------------------------------------------------------
//	getSize()
//		Collects the number of bytes used by this tree
//		bd_tree.
//----------------------------------------------------------------------

int ANNkd_tree::getByteSize()
{
	// Size of this class
	int result = sizeof(*this);
	// Sizeof root
	result += root->getByteSize();
	// Sizeof pidx
	if (pidx != NULL)
		result += n_pts*sizeof(ANNidx);
	// Sizeof boxes
	if (bnd_box_lo != NULL)
		result += dim*sizeof(ANNcoord);
	if (bnd_box_hi != NULL)
		result += dim*sizeof(ANNcoord);
	// Return the result!
	return result;
...
{
	if (root != NULL) ann_delete_obj(root);
	if (pidx != NULL) ann_delete_arr(pidx);
...
{
	if (g_KD_TRIVIAL != NULL) {
		ann_delete_obj(g_KD_TRIVIAL);
		g_KD_TRIVIAL = NULL;
	}
}
		...
		ANNidxArray pi)					// point indices
{
	...
	if (pi == NULL) {					// point indices provided?
		pidx = ann_new_arr(ANNidx, n);			// no, allocate space for point indices
		for (int i = 0; i < n; i++) {
	...
	if (g_KD_TRIVIAL == NULL)				// no trivial leaf node yet?
		g_KD_TRIVIAL = ann_new(ANNkd_leaf(0, g_IDX_TRIVIAL));	// allocate it
}
	...
	ANNkd_splitter		splitter)		// splitting routine
{
	if (n <= bsp) {						// n small, make a leaf node
		if (n == 0)						// empty leaf node
			return g_KD_TRIVIAL;			// return (canonical) empty leaf
		else							// construct the node and return
			return ann_new(ANNkd_leaf(n, pidx));
	}
		...
		ANNkd_split *ptr = ann_new(ANNkd_split(cd, cv, lv, hv, lo, hi));
