...
public:
	...
	virtual void ann_search(ANNparams *params, ANNdist) = 0;		// tree search
	virtual void ann_pri_search(ANNparams *params, ANNdist) = 0;	// priority search
	virtual void ann_FR_search(ANNparams *params, ANNdist) = 0;	// fixed-radius search
	...
	virtual int getByteSize() = 0;				// return size of this node in bytes
	ANN_RT_DESTROY(ANNkd_node)
...
public:
	...
	virtual void ann_search(ANNparams *params, ANNdist);			// standard search
	virtual void ann_pri_search(ANNparams *params, ANNdist);		// priority search
	virtual void ann_FR_search(ANNparams *params, ANNdist);		// fixed-radius search
	// Added by Erik Schuitema
	virtual int getByteSize();					// return size of this node in bytes
	ANN_RT_DESTROY(ANNkd_leaf)
};
...
extern ANNkd_leaf *g_KD_TRIVIAL;					// trivial (empty) leaf node
...
public:
	...
	~ANNkd_split()						// destructor
		{
			if (child[ANN_LO]!= NULL && child[ANN_LO]!= g_KD_TRIVIAL)
				ann_delete_obj(child[ANN_LO]);
			if (child[ANN_HI]!= NULL && child[ANN_HI]!= g_KD_TRIVIAL)
				ann_delete_obj(child[ANN_HI]);
		}
