	...
	double				eps)			// error bound (ignored)
{
	...
	m_params.prMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating ops

	m_params.prDim = dim;						// copy arguments to static equivs
	m_params.prQ = q;
	m_params.prPts = pts;
	m_params.ptsVisited = 0;					// initialize count of points visited

	m_params.prPointMK = ann_new(ANNmin_k(k));		// create set for closest k points

										// distance to root box
	ANNdist box_dist = annBoxDistance(q,
				bnd_box_lo, bnd_box_hi, dim);

	m_params.prBoxPQ = ann_new(ANNpr_queue(n_pts));// create priority queue for boxes
	m_params.prBoxPQ->insert(box_dist, root); // insert root in priority queue

	while (m_params.prBoxPQ->non_empty() &&
		(!(g_maxPtsVisited != 0 && m_params.ptsVisited > g_maxPtsVisited))) {
		PQinfo np;						// next box from prior queue of type ANNkd_ptr

										// extract closest box from queue
		m_params.prBoxPQ->extr_min(box_dist, np);

		ANN_FLOP(2)						// increment floating ops
		if (box_dist*m_params.prMaxErr >= m_params.prPointMK->max_key())
			break;

		((ANNkd_ptr)np)->ann_pri_search(&m_params, box_dist);	// search this subtree.
	}

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = m_params.prPointMK->ith_smallest_key(i);
		nn_idx[i] = m_params.prPointMK->ith_smallest_info(i);
	}

	ann_delete_obj(m_params.prPointMK);				// deallocate closest point set
	ann_delete_obj(m_params.prBoxPQ);					// deallocate priority queue
}
...
void ANNkd_split::ann_pri_search(ANNparams *params, ANNdist box_dist)
{
	ANNdist new_dist;					// distance to child visited later
										// distance to cutting plane
	ANNcoord cut_diff = params->prQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		ANNcoord box_diff = cd_bnds[ANN_LO] - params->prQ[cut_dim];
		...
		if (child[ANN_HI] != g_KD_TRIVIAL)// enqueue if not trivial
			params->prBoxPQ->insert(new_dist, child[ANN_HI]);
										// continue with closer child
		child[ANN_LO]->ann_pri_search(params, box_dist);
	}
	else {								// right of cutting plane
		ANNcoord box_diff = params->prQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		new_dist = (ANNdist) ANN_SUM(box_dist,
		...
		if (child[ANN_LO] != g_KD_TRIVIAL)// enqueue if not trivial
			params->prBoxPQ->insert(new_dist, child[ANN_LO]);
										// continue with closer child
		child[ANN_HI]->ann_pri_search(params, box_dist);
	}
...
void ANNkd_leaf::ann_pri_search(ANNparams *params, ANNdist box_dist)
{
	...
	min_dist = params->prPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = params->prPts[bkt[i]];			// first coord of next data point
		qq = params->prQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < params->prDim; d++) {
		...
		if (d >= params->prDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			params->prPointMK->insert(dist, bkt[i]);
			min_dist = params->prPointMK->max_key();
		}
	}
	ANN_LEAF(1)							// one more leaf node visited
	ANN_PTS(n_pts)						// increment points visited
  params->ptsVisited += n_pts;				// increment number of points visited
}
