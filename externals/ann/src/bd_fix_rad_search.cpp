...
void ANNbd_shrink::ann_FR_search(ANNparams *params, ANNdist box_dist)
{
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(params->kdFRQ)) {			// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(params->kdFRQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_FR_search(params, inner_dist);// search inner child first
		child[ANN_OUT]->ann_FR_search(params, box_dist);// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_FR_search(params, box_dist);// search outer child first
		child[ANN_IN]->ann_FR_search(params, inner_dist);// ...then outer child
	}
