...
void ANNbd_shrink::ann_pri_search(ANNparams *params, ANNdist box_dist)
{
	ANNdist inner_dist = 0;						// distance to inner box
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(params->prQ)) {				// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(params->prQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		if (child[ANN_OUT] != g_KD_TRIVIAL)		// enqueue outer if not trivial
		  params->prBoxPQ->insert(box_dist,child[ANN_OUT]);
												// continue with inner child
		child[ANN_IN]->ann_pri_search(params, inner_dist);
	}
	else {										// if outer box is closer
		if (child[ANN_IN] != g_KD_TRIVIAL)		// enqueue inner if not trivial
		  params->prBoxPQ->insert(inner_dist,child[ANN_IN]);
												// continue with outer child
		child[ANN_OUT]->ann_pri_search(params, box_dist);
	}
