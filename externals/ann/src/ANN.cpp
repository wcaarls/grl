...
#ifdef ANN_REALTIME
CMemoryPool* gAnnMemoryPool = NULL;
...
{
	ANNpoint p = ann_new_arr(ANNcoord, dim);
...
{
	ANNpointArray pa = ann_new_arr(ANNpoint, n);			// allocate points
	ANNpoint	  p  = ann_new_arr(ANNcoord, n*dim);		// allocate space for coords
	for (int i = 0; i < n; i++) {
		pa[i] = &(p[i*dim]);
	}
	return pa;
...
{
	ann_delete_arr(pa[0]);							// dealloc coordinate storage
	ann_delete_arr(pa);								// dealloc points
	pa = NULL;
}
