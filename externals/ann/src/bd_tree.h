...
public:
	...
	~ANNbd_shrink()						// destructor
		{
			if (child[ANN_IN]!= NULL && child[ANN_IN]!=  g_KD_TRIVIAL)
				ann_delete_obj(child[ANN_IN]);
			if (child[ANN_OUT]!= NULL&& child[ANN_OUT]!= g_KD_TRIVIAL)
				ann_delete_obj(child[ANN_OUT]);
			if (bnds != NULL)
				ann_delete_obj_arr(bnds, n_bnds);			// delete bounds
		}
