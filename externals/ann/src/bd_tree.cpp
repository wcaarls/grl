...
int ANNbd_shrink::getByteSize()
{
	int result = sizeof(*this);
	// TODO: fix this, it is incomplete (bounding box array...)
	if (child[0] != NULL)
	...
	ANNshrinkRule		shrink)			// shrinking rule
{
	...
	if (decomp == SPLIT) {				// split selected
		...
		return ann_new(ANNkd_split(cd, cv, lv, hv, lo, hi));
	}
		...
		return ann_new(ANNbd_shrink(n_bnds, bnds, in, out));
	}
} 
