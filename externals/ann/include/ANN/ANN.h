...
#if defined(WIN32) && defined(SHARED_LIBRARY)
  //----------------------------------------------------------------------
...
//  Will only be enabled by compiler flag -DANN_REALTIME (or #define ANN_REALTIME)
//----------------------------------------------------------------------
#include <ANN/ANNrt.h>
...
// wcaarls, 11-4-2012: group globals
struct ANNparams
{
    ANNparams();

    // from ANN
    int                 ptsVisited;     // number of pts visited in search

    // from kd_search
    int                 kdDim;          // dimension of space
    ANNpoint            kdQ;            // query point
    double              kdMaxErr;       // max tolerable squared error
    ANNpointArray       kdPts;          // the points
    class ANNmin_k*     kdPointMK;      // set of k closest points

    // from kd_pr_search
    double              prEps;          // the error bound
    int                 prDim;          // dimension of space
    ANNpoint            prQ;            // query point
    double              prMaxErr;       // max tolerable squared error
    ANNpointArray       prPts;          // the points
    class ANNpr_queue*  prBoxPQ;        // priority queue for boxes
    class ANNmin_k*     prPointMK;      // set of k closest points

    // from kd_fix_rad_search
    int                 kdFRDim;        // dimension of space
    ANNpoint            kdFRQ;          // query point
    ANNdist             kdFRSqRad;      // squared radius search bound
    double              kdFRMaxErr;     // max tolerable squared error
    ANNpointArray       kdFRPts;        // the points
    class ANNmin_k*     kdFRPointMK;    // set of k closest points

    int                 kdFRPtsVisited; // total points visited
    int                 kdFRPtsInRange; // number of points in the range
};
...
public:
	...
	// Added by Erik Schuitema
	virtual int getByteSize();
};								

// Shallow tree copy: copies only the data that is modified
// when doing a search (for concurrent searches)
class DLL_API ANNkd_tree_copy: public ANNkd_tree
{
  public:
    ANNkd_tree_copy(const ANNkd_tree &other) :
      ANNkd_tree(other)
    {
      if (bnd_box_lo != NULL) bnd_box_lo = annCopyPt(dim, bnd_box_lo);
      if (bnd_box_hi != NULL) bnd_box_hi = annCopyPt(dim, bnd_box_hi);
    }
