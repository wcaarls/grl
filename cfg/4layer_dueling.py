...
h2s = Dense(layer2_size/2, activation='relu', name='h2s')(h1)
h2a = Dense(layer2_size/2, activation='relu', name='h2a')(h1)
outs = Dense(1, activation='linear', name='outs')(h2s)
outa = Dense(outputs, activation='linear', name='outa')(h2a)
outp = tf.identity(outs + outa - tf.math.reduce_mean(outa, 1, keep_dims=True), name='output')
